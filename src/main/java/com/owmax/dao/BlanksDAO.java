...
/**
 	* A data access object (DAO) providing persistence and search support for Blanks entities.
...
public class BlanksDAO extends BaseHibernateDAO  {
    ...
    public Blanks findById( Integer id) {
        log.debug("getting Blanks instance with id: " + id);
        try {
            Blanks instance = (Blanks) getSession()
                    .get("com.owmax.model.Blanks", id);
            return instance;
        } catch (RuntimeException re) {
    ...
    public Blanks merge(Blanks detachedInstance) {
        ...
        try {
            Blanks result = (Blanks) getSession()
